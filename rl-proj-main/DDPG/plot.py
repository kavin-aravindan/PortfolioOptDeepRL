...
def plot_metrics(episodes, mean_rewards, mean_devs, cumulative_rewards, 
    ...
    # Plot 4: Actor Loss
    axs[3].plot(episodes, actor_losses, marker='.', linestyle='-', color='deepskyblue', label='Critic Loss')
    axs[3].set_ylabel("Critic Loss")
    axs[3].set_title("Critic Loss per Episode")
    axs[3].grid(True)
    axs[3].legend()

    # Plot 5: Critic Loss
    axs[4].plot(episodes, critic_losses, marker='.', linestyle='-', color='orangered', label='Actor Loss')
    axs[4].set_xlabel("Episode Number") # X-axis label on the last plot
    axs[4].set_ylabel("Actor Loss")
    axs[4].set_title("Actor Loss per Episode")
    axs[4].grid(True)
    axs[4].legend()
...
if __name__ == "__main__":
    log_filepath = "outfinal.txt" # Make sure this is your log file's name
    ...
    if episodes:
        ...
        num_episodes_to_show = 300 # Example: plot all parsed episodes
